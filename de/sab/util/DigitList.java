/*
 * @(#)DigitList.java 1.28 03/01/23 Copyright 2003 Sun Microsystems, Inc. All
 ...
 * IBM Corp. 1996 - 1998 - All Rights Reserved The original version of this
 * source code and documentation is copyrighted and owned by Taligent, Inc., a
 * wholly-owned subsidiary of IBM. These materials are provided under terms of a
 * License Agreement between Taligent and Sun. This technology is protected by
 * multiple US and International patents. This notice and attribution to
 * Taligent may not be removed. Taligent is a registered trademark of Taligent,
 * Inc.
 */
 ...
 * currency, and so on. A DigitList is really a representation of a floating
 * point value. It may be an integer value; we assume that a double has
 * sufficient precision to represent all digits of a long. The DigitList
 * representation consists of a string of characters, which are the digits radix
 * 10, from '0' to '9'. It also has a radix 10 exponent associated with it. The
 * value represented by a DigitList object can be computed by mulitplying the
 * fraction f, where 0 <= f < 1, derived by placing all the digits of the list
 * to the right of the decimal point, by 10^exponent.
 * 
 * @see Locale
 * @see Format
 * @see NumberFormat
...
final class DigitList implements Cloneable
{
	 ...
	 * digits in Digits to the right of the decimal. DigitList is normalized, so
	 * if it is non-zero, figits[0] is non-zero. We don't allow denormalized
	...
	/**
	 * Appends a digit to the list. Ignores all digits over MAX_COUNT, since
	 * they are not significant for either longs or doubles.
	 */
	public void append(char digit)
	{
		if (count < MAX_COUNT) digits[count++] = digit;
	}
	...
	final void set(double source, int maximumDigits, boolean fixedPoint)
	{
		...
		for (int i = 0; i < rep.length;)
		{
			...
			else if (count < MAX_COUNT)
			{
				...
				if (nonZeroDigitSeen) digits[count++] = c;
			}
		}
	 ...
	 *            <code>count-1</code>. If 0, then all digits are rounded
	 *            away, and this method returns true if a one should be
	 *            generated (e.g., formatting 0.09 with "#.#").
	 ...
	 *            maximumDigits is lower than the number of significant digits
	 *            in source, the representation will be rounded. Ignored if <=
	 *            0.
	 */
	...
	{
		if (decimalAt != count || count != MAX_COUNT) return false;
